    ...
    if time.localtime().tm_min==timer or time.localtime().tm_min==test_timer:
        ...
        for items in symbols:
            ...
            if numpy.isnan(result_4h.iloc[result_4h.shape[0] - 1,]["Buy"]) == False:
                ...
                if str(round(result_4h.iloc[result_4h.shape[0] - 1,]["Buy"], 3)) != str(last_buy_signal[items]):
                    print("bot tetiklendi")
                    bot.send_message(chat_id,items+": buy at ----> "+str(round((result_4h.iloc[result_4h.shape[0] - 1,]["Buy"]),3)))
                    last_buy_signal[items] = round((result_4h.iloc[result_4h.shape[0] - 1,]["Buy"]),3)
            else:
                databaseFile.sql_update(databaseFile.connector, items, str(result_4h["Time"].iloc[-1]),str(round((result_4h.iloc[result_4h.shape[0] - 1,]["Sell"]), 3)), status="sell")
                databaseFile.sql_update(databaseFile.connector, items, str(result_4h["Time"].iloc[-1]), "No signal", status="sell")
                print(items+": Sell 4H ---->:"+str(round((result_4h.iloc[result_4h.shape[0] - 1,]["Sell"]),3)))
                if str(round(result_4h.iloc[result_4h.shape[0] - 1,]["Sell"], 3)) != str(last_sell_signal[items]):
                    print("bot tetiklendi")
                    bot.send_message(chat_id, items + ": sell at ----> " + str(round((result_4h.iloc[result_4h.shape[0] - 1,]["Sell"]), 3)))
                    last_sell_signal[items] = round((result_4h.iloc[result_4h.shape[0] - 1,]["Sell"]),3)
    finish = datetime.now()
    sure = finish-start
