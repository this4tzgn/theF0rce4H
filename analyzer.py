...
def reshaper():
    symbols_infunc = ["BTC: ","ETH: ","CAKE: ","NEO: ","BNB: ","BAKE: ","DOT: ","EOS: ","ETC: ","ADA: ","BCH: ","LTC: ","XRP: ","DOGE: ","SOL: ","LUNA: "]
    for i in range(0,len(symbols)):
        buy_signal[i] = databaseFile.selectTime(databaseFile.connector,symbols[i],"sell")+"---->"+symbols_infunc[i] + databaseFile.selectSignal(databaseFile.connector,symbols[i],"buy")
    with open("buy_signals.txt", "w", encoding="utf-8") as file:
                for item in buy_signal:
                    file.write(str(item))
                    file.write("\n")        
    for i in range(0,len(symbols)):
        sell_signal[i] = databaseFile.selectTime(databaseFile.connector,symbols[i],"sell")+"---->"+symbols_infunc[i] + databaseFile.selectSignal(databaseFile.connector,symbols[i],"sell")
        ...
        for items in symbols:
            ...
            if numpy.isnan(result.iloc[result.shape[0] - 1,]["Buy"]) == False:
                ...
                if str(round(result.iloc[result.shape[0] - 1,]["Buy"], 3)) != str(last_buy_signal[items]):
                    ...
                    bot.send_message(telegramBotAfgan.chat_id,items+": buy at ----> "+str(round((result.iloc[result.shape[0] - 1,]["Buy"]),3)))
                ...
                if str(round(result.iloc[result.shape[0] - 1,]["Sell"], 3)) != str(last_sell_signal[items]):
                    print("bot tetiklendi")
                    bot.send_message(telegramBotAfgan.chat_id, items + ": sell at ----> " + str(round((result.iloc[result.shape[0] - 1,]["Sell"]), 3)))
